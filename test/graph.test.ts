...
import { GraphBuilder, GraphNode } from '../src/core/graph';
...
describe('GraphBuilder', () => {
    ...
    new StackManifest({
      ...
      metadata: {
        'stack/example-bucket/Resource': {
          stackPath: 'stack',
          id: 'examplebucketC9DFA43E',
        },
        'stack/example-bucket/Policy/Resource': {
          stackPath: 'stack',
          id: 'examplebucketPolicyE09B485E',
        },
        ...
        'stack/output-bucket/Resource': {
          stackPath: 'stack',
          id: 'outputbucketC9DFA43E',
        },
        'stack/output-bucket/Policy/Resource': {
          stackPath: 'stack',
          id: 'outputbucketPolicyE09B485E',
        },
      },
      nestedStacks: {
        'stack/nested': {
  ...
  test.each([
    ...
    [
      'stack/nested/example-bucket',
      {
        ...
        incomingEdges: [
          'stack/nested/example-bucket/Resource',
          'stack/nested/example-bucket/Policy',
        ],
    ...
    [
      'stack/nested/example-bucket/Resource',
      {
        ...
        incomingEdges: [
          'stack/nested/example-bucket/Policy/Resource',
          'stack/output-bucket/Resource',
        ], // the incoming edge for 'stack/output-bucket/Resource' is the stack output
    ...
    [
      'stack/output-bucket/Resource',
      {
        ...
        outgoingEdges: [
          ...
          'stack/nested',
          'stack/nested/example-bucket/Resource',
        ], // the outgoing edge for 'stack/nested/example-bucket/Resource' is the nested stack output
      },
    ],
    [
      'stack/output-bucket/Policy',
  ...
  ])('Parses the graph correctly: %s', (constructPath, expected) => {
    const actual = nodes.nodes.find(
      (node) => node.construct.path === constructPath,
    );
    ...
    expect(new Set(edgesToArray(actual!.incomingEdges))).toEqual(
      new Set(expected.incomingEdges),
    );
    expect(new Set(edgesToArray(actual!.outgoingEdges))).toEqual(
      new Set(expected.outgoingEdges),
    );
  });
  ...
  test.each([
    ...
    [
      ...
      createStackManifest({
        resource2Props: {
          SomeProp: {
            'Fn::Sub': ['www.${Domain}', { Domain: { Ref: 'resource1' } }],
          },
        },
      }),
    ],
      ...
      createStackManifest({
        resource2Props: {
          SomeProp: {
            'Fn::Sub': [
              'www.${Domain}',
              { Domain: { 'Fn::GetAtt': ['resource1', 'Arn'] } },
            ],
          },
        },
      }),
    ...
    expect(edgesToArray(graph[2].outgoingEdges)).toEqual([
      'stack',
      'stack/resource-1',
    ]);
  });
...
test.each([
  ...
  [
    ...
    createNestedStackManifest({
      nestedStackResourceProps: {
        Parameters: {
          parentRef: {
            'Fn::Sub': ['test.${Domain}', { Domain: { Ref: 'parent' } }],
          },
        },
      },
      nestedStackParameters: {
      ...
      nestedResourceProps: {
        BucketName: {
          'Fn::Sub': ['sub.${Domain}', { Domain: { Ref: 'parentRef' } }],
        },
      },
    }),
    {
  ...
  [
    ...
    createNestedStackManifest({
      nestedStackOutputs: {
        nestedStackOutput: {
          Value: {
            'Fn::Sub': ['test.${Domain}', { Domain: { Ref: 'child' } }],
          },
        },
      },
      outputResourceProps: {
  ...
  const graph = GraphBuilder.build(stackManifest).nodes;
  const childNode = graph.find(
    (node) => node.construct.path === 'stack/nested/example-bucket/Resource',
  );
  const parentNode = graph.find(
    (node) => node.construct.path === 'stack/example-bucket/Resource',
  );
  const outputNode = graph.find(
    (node) => node.construct.path === 'stack/output-bucket/Resource',
  );
  const nestedStackNode = graph.find(
    (node) => node.construct.path === 'stack/nested',
  );

  expect(new Set(edgesToArray(parentNode!.outgoingEdges))).toEqual(
    new Set(['stack/example-bucket']),
  );
  expect(new Set(edgesToArray(childNode!.outgoingEdges))).toEqual(
    new Set(['stack/nested/example-bucket']),
  );
  expect(new Set(edgesToArray(outputNode!.incomingEdges))).toEqual(
    new Set(['stack/output-bucket/Policy/Resource']),
  );
...
test('vpc with ipv6 cidr block', () => {
  ...
  expect(
    Array.from(nodes[2].incomingEdges.values())[0].resourceAddress,
  ).toEqual({ stackPath: 'stack', id: 'other' });
  expect(
    Array.from(nodes[3].outgoingEdges.values())[1].resourceAddress,
  ).toEqual({ stackPath: 'stack', id: 'cidr' });
});
...
test('parses custom resources', () => {
  ...
  const deployWebsiteCR = graph.nodes.find(
    (node) =>
      node.resourceAddress?.id === 'DeployWebsiteCustomResourceD116527B',
  );
  ...
  const testRole = graph.nodes.find(
    (node) => node.resourceAddress?.id === 'CustomResourceRoleAB1EF463',
  );
    ...
    if (tree.children) {
      tree.children = Object.fromEntries(
        Object.entries(tree.children)
          .filter(([_, value]) => value.path !== constructPath)
          .map(([key, value]) => [
            key,
            deleteResourceFromTree(value, constructPath),
          ]),
      );
    }
    return tree;
