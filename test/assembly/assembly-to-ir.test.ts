import { CloudFormationTemplate, NestedStackTemplate } from '../../src/core';
import {
...
describe('convertAssemblyToProgramIr', () => {
  ...
  test('convertAssemblyDirectoryToProgramIr loads manifest via reader', () => {
    ...
    const spy = jest
      .spyOn(AssemblyManifestReader, 'fromDirectory')
      .mockReturnValue(fakeReader);
  ...
  test('convertStageInAssemblyDirectoryToProgramIr loads nested manifest', () => {
    ...
    const spy = jest
      .spyOn(AssemblyManifestReader, 'fromDirectory')
      .mockReturnValue(rootReader);

    const program = convertStageInAssemblyDirectoryToProgramIr(
      '/fake/assembly',
      'DevStage',
    );
  ...
  test('convertAssemblyToProgramIr skips stacks outside the filter set', () => {
    ...
    const reader = {
      stackManifests: [manifestA, manifestB],
    } as unknown as AssemblyManifestReader;

    const program = convertAssemblyToProgramIr(
      reader,
      new Set(['FilteredStack']),
    );
  ...
  test('Fn::ImportValue references resolve to stack output references', () => {
    ...
    const program = convertAssemblyToProgramIr(reader);
    const consumerStack = program.stacks.find(
      (stack) => stack.stackId === 'ConsumerStack',
    );
    expect(consumerStack).toBeDefined();
    const topic = consumerStack!.resources.find(
