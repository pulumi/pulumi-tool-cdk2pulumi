...
- Emits richer IR nodes for structural intrinsics. For example `Fn::Join`, `Fn::Sub`, and other string-building constructs become `ConcatValue` records whenever they cannot be fully reduced to a literal, ensuring the serializer recreates the `fn::join`/`${name}` mix correctly. Intrinsics that now map cleanly to provider helpers such as `Fn::Cidr` and `Fn::GetAZs` become `InvokeValue` records, and symbolic list indexing is preserved as `SelectValue`.
