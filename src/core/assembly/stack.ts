...
export class StackManifest {
   ...
   */
  public static getNestedStackPath(
    nestedStackResourcePath: string,
    logicalId: string,
  ): string {
   ...
   *
   * @param constructPath - The construct path
   * @returns the metadata of the resource
   * @throws error if the construct path does not relate to a CFN resource
   */
  public resourceAddressForPath(constructPath: string): StackAddress {
    if (constructPath in this.metadata) {
      return this.metadata[constructPath];
    }
    throw new Error(`Could not find stack address for path ${constructPath}`);
  }
