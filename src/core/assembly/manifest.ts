...
export class AssemblyManifestReader {
   ...
   */
  public static fromDirectoryWithTree(
    dir: string,
    tree: ConstructTree,
  ...
  constructor(
    ...
    try {
      if (treeOverride) {
        ...
        const fullTree = fs.readJsonSync(
          path.resolve(this.directory, 'tree.json'),
        );
        if (!fullTree.tree) {
          throw new Error(
            `Invalid tree.json found ${JSON.stringify(fullTree)}`,
          );
        }
    ...
    for (const [artifactId, artifact] of Object.entries(
      ...
      if (artifact.type === ArtifactType.AWS_CLOUDFORMATION_STACK) {
        if (!artifact.properties || !('templateFile' in artifact.properties)) {
          throw new Error(
            'Invalid CloudFormation artifact. Cannot find the template file',
          );
        }
        ...
        try {
          template = fs.readJSONSync(
            path.resolve(this.directory, templateFile),
          );
        } catch (e) {
          throw new Error(
            `Failed to read CloudFormation template at path: ${templateFile}: ${e}`,
          );
        }
   ...
   */
  private loadNestedStacks(
    resources: { [logicalIds: string]: CloudFormationResource } | undefined,
  ): {
    ...
    return Object.entries(resources ?? {})
      .filter(([_, resource]) => {
        return (
          ...
          resource.Metadata?.['aws:asset:path']
        );
      })
      .reduce(
        (acc, [logicalId, resource]) => {
          ...
          if (!cdkPath) {
            throw new Error(
              `Expected the nested stack ${logicalId} to have a 'aws:cdk:path' metadata entry`,
            );
          }
    ...
    for (const [metadataId, metadataEntry] of Object.entries(
      ...
      metadataEntry.forEach((meta) => {
        if (meta.type === ArtifactMetadataEntryType.LOGICAL_ID) {
          ...
          const realPath = metadataId.startsWith('/')
            ? metadataId.substring(1)
            : metadataId;
          ...
          for (const stackPrefix of stackPrefixes) {
            if (
              stackPrefix.length > (stackPath?.length ?? 0) &&
              realPath.startsWith(stackPrefix)
            ) {
          ...
          if (!stackPath && stackPath !== '') {
            throw new Error(
              `Failed to determine the stack path for resource at path ${realPath}`,
            );
          }
          ...
          const logicalId =
            typeof meta.data === 'string'
              ? meta.data
              : (meta.data as { logicalId?: string })?.logicalId;
          if (!logicalId) {
            throw new Error(
              `Failed to determine logical id for resource at path ${realPath}`,
            );
          }
  ...
  public loadNestedAssembly(stageIdentifier: string): AssemblyManifestReader {
    for (const [artifactId, artifact] of Object.entries(
      ...
      const props = artifact.properties as
        | NestedAssemblyArtifactProperties
        | undefined;
      if (!props?.directoryName) {
        throw new Error(
          `Nested assembly '${artifactId}' is missing the 'directoryName' property`,
        );
      }

      const stagePath =
        props.displayName ?? artifact.displayName ?? stageIdentifier;
      const stageTree = this.findConstructTreeNode(stagePath);
      if (!stageTree) {
        throw new Error(
          `Could not locate construct tree node for nested assembly '${stageIdentifier}'`,
        );
      }
    ...
    throw new Error(
      `Unknown stage '${stageIdentifier}' in assembly at '${this.directory}'`,
    );
  }
  ...
  private matchesStageIdentifier(
    ...
    const props = artifact.properties as
      ...
      | undefined;
    return (
      props?.displayName === identifier || props?.directoryName === identifier
    );
  }

  private getStackTreeNode(
    artifactId: string,
    artifact: ArtifactManifest,
  ): ConstructTree {
    ...
    const displayNameNode = this.findConstructTreeNode(
      artifact.displayName ?? artifactId,
    );
    if (displayNameNode) {
    ...
    throw new Error(
      `Could not find construct tree entry for stack '${artifactId}'`,
    );
  }
  ...
  private findConstructTreeNode(pathValue?: string): ConstructTree | undefined {
    ...
    if (
      segments[0] === this.tree.id ||
      (normalizedTreePath && segments[0] === normalizedTreePath)
    ) {
