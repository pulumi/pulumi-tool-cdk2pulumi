...
function normalizeResourceTypeComponents(
  resourceType: string,
...
export function lowerAcronyms(s: string): string {
  ...
  return (
    ...
    s.slice(startIndex, endIndex).toLowerCase() +
    lowerAcronyms(s.slice(endIndex))
  );
}
...
function firstUppercaseAcronym(s: string): [number, number] {
  ...
  // Note: we've defined uppercase to be ASCII [A-Z] so index math is safe here
  if (
    !(endIndex === s.length || startsWithIsolatedLowercaseS(s.slice(endIndex)))
  ) {
    endIndex = endIndex - 1;
  }
...
export function findFirstRunOfUppercase(
  s: string,
  minLength: number,
): [number, number] {
  let startIndex = -1;
