...
function typeFromCfn(cfnType: string): string {
  ...
  if (typeParts.length !== 3) {
    throw new CdkAdapterError(
      `Expected cfn type in format 'AWS::Service::Resource', got ${cfnType}`,
    );
  }
  return typeParts[2];
}
...
export class GraphBuilder {
  ...
  private static isCustomResource(
    node: ConstructTree,
    parent?: ConstructInfo,
  ): boolean {
    ...
    return (
      parent?.constructInfo?.fqn === 'aws-cdk-lib.CustomResource' &&
      node.id === 'Default'
    );
  }
  ...
  private parseTree(tree: ConstructTree, parent?: ConstructInfo) {
    ...
    if (tree.attributes && 'aws:cdk:cloudformation:type' in tree.attributes) {
      ...
      const resource = this.stack.resourceWithLogicalId(
        resourceAddress.stackPath,
        resourceAddress.id,
      );
      ...
      if (resource.Type === 'AWS::EC2::VPC') {
        this.vpcNodes.set(resourceAddress, {
          vpcNode: node,
          vpcCidrBlockNode: undefined,
        });
      }
    } else if (
      node.construct.constructInfo?.fqn === 'aws-cdk-lib.CfnResource'
    ) {
    ...
    nestedStacks.forEach((nestedStack) => {
      ...
      this.cfnElementNodes.set(nestedStack.resourceAddress, nestedNode);
      this.constructNodes.set(nestedStackConstruct, nestedNode);
      this.nestedStackNodes.set(nestedStack.node.path, nestedNode);
      ...
      Object.values(tree.children)
        .filter(
          (child) =>
            !nestedStacks.find(
              (nestedStack) =>
                nestedStack.node.id === child.id ||
                child.id === `${nestedStack.node.id}.NestedStack`,
            ),
        )
  ...
  private findNestedStacks(tree: ConstructTree): NestedStackData[] {
    ...
    const nestedStackResources = Object.values(tree.children ?? {})
      .map((child) => {
        ...
        return Object.values(child.children ?? {}).find((c) => {
          return (
            c.attributes &&
            'aws:cdk:cloudformation:type' in c.attributes &&
            c.attributes['aws:cdk:cloudformation:type'] ===
              'AWS::CloudFormation::Stack'
          );
        });
      })
    ...
    const nestedStacks = nestedStackResources
      .map((nestedStackResourceNode) => {
        ...
        const resourceAddress = this.stack.resourceAddressForPath(
          nestedStackResourceNode.path,
        );
        const resource = this.stack.resourceWithLogicalId(
          resourceAddress.stackPath,
  ...
  private _build(): Graph {
    ...
    Object.entries(this.stack.stacks).map(([stackPath, template]) => {
      Object.entries(template.Resources ?? {}).map(([logicalId, resource]) => {
        if (!this.cfnElementNodes.has({ id: logicalId, stackPath })) {
          warn(
            `CDK resource ${logicalId} (${resource.Type}) was not mapped to a Pulumi resource.`,
          );
    ...
    for (const [construct, node] of this.constructNodes) {
      ...
      if (node.resource && node.resourceAddress) {
        ...
        const dependsOn =
          typeof node.resource.DependsOn === 'string'
            ? [node.resource.DependsOn]
            : node.resource.DependsOn;
  ...
  private addEdgesForCfnResource(
    obj: any,
    source: GraphNode,
    stackPath?: string,
  ): void {
    ...
    if (!targetLogicalId.startsWith('AWS::')) {
      ...
      if (targetNode === undefined) {
        ...
        if (!this.stack.isRootStack(stackPath)) {
          ...
          if (
            nestedStackNode?.resource?.Properties?.Parameters?.[targetLogicalId]
          ) {
            debug(
              `Parameter ${targetLogicalId} is set by the parent stack ${stackPath}`,
            );
            return;
          }
        }
  ...
  private addEdgesForIntrinsic(
    ...
    switch (fn) {
      case 'Fn::GetAtt': {
        ...
        const targetNodePath = this.cfnElementNodes.get(targetResourceAddress)
          ?.construct?.path;
        ...
        if (targetNodePath && targetNodePath in this.stack.stacks) {
          ...
          if (nestedStack.Outputs?.[outputName]?.Value) {
            this.addEdgesForCfnResource(
              nestedStack.Outputs[outputName].Value,
              source,
              targetNodePath,
            );
          }
        ...
        if (
          ...
          this.vpcNodes.get(targetResourceAddress)?.vpcCidrBlockNode
            ?.resourceAddress
        ) {
          logicalId = this.vpcNodes.get(targetResourceAddress)!
            .vpcCidrBlockNode!.resourceAddress!.id;
        }
      ...
      case 'Fn::Sub':
        {
          ...
          for (const part of parseSub(template).filter(
            (p) => p.ref !== undefined,
          )) {
