...
} from '../converters/intrinsic-value-adapter';
...
> {
  ...
  private resolveRefAttribute(
    ...
    if (cfRef.notSupported) {
      throw new Error(
        `Ref intrinsic is not supported for the ${request.mapping.resourceType} resource type`,
      );
    }
  ...
  private getDefaultPropertyName(
