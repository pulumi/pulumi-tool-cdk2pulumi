...
export interface IrIntrinsicResolverProps {
  ...
  lookupStackOutputByExportName?: (
    exportName: string,
  ) => StackOutputReference | undefined;
}
...
export class IrIntrinsicResolver {
  ...
  private readonly lookupStackOutputByExportName?: (
    exportName: string,
  ) => StackOutputReference | undefined;
  ...
  private resolveImportValue(param: any): PropertyValue | undefined {
    ...
    if (typeof resolved !== 'string') {
      throw new Error(
        'Fn::ImportValue only supports string literal export names at this time',
      );
    }
    ...
    if (!reference) {
      throw new Error(
        `Unable to resolve export '${resolved}' referenced by Fn::ImportValue in ${this.stackPath}`,
      );
    }
  ...
  private resolveSub(params: any): PropertyValue | undefined {
    const [template, variables] =
      typeof params === 'string' ? [params, undefined] : params;
    ...
    for (const part of parseSub(template)) {
      ...
      if (part.ref) {
        const resolved = this.resolveSubReference(
          part.ref.id,
          part.ref.attr,
          resolvedVars,
        );
  ...
  private resolveFindInMap(params: [any, any, any]): PropertyValue | undefined {
    if (!Array.isArray(params) || params.length !== 3) {
      throw new Error(
        `Fn::FindInMap requires exactly 3 parameters, got ${params.length}`,
      );
    }
    ...
    if (!(secondKey in topLevel)) {
      throw new Error(
        `Key ${secondKey} not found in mapping ${mappingName}.${topKey}`,
      );
    }
...
function isIf(value: any): value is { 'Fn::If': [string, any, any] } {
  return (
    typeof value === 'object' &&
    value !== null &&
    Array.isArray(value['Fn::If']) &&
    value['Fn::If'].length === 3
  );
}
...
function isSplit(value: any): value is { 'Fn::Split': [string, any] } {
  return (
    typeof value === 'object' &&
    value !== null &&
    Array.isArray(value['Fn::Split']) &&
    value['Fn::Split'].length === 2
  );
}
...
function isSelect(value: any): value is { 'Fn::Select': [any, any] } {
  return (
    typeof value === 'object' &&
    value !== null &&
    Array.isArray(value['Fn::Select']) &&
    value['Fn::Select'].length === 2
  );
}
...
function isBase64(value: any): value is { 'Fn::Base64': any } {
  return (
    typeof value === 'object' &&
    value !== null &&
    value['Fn::Base64'] !== undefined
  );
}

function isFindInMap(
  value: any,
): value is { 'Fn::FindInMap': [any, any, any] } {
  return (
    typeof value === 'object' &&
    value !== null &&
...
function isImportValue(value: any): value is { 'Fn::ImportValue': any } {
  return (
    typeof value === 'object' &&
    value !== null &&
    value['Fn::ImportValue'] !== undefined
  );
}
...
function isEquals(value: any): value is { 'Fn::Equals': [any, any] } {
  return (
    typeof value === 'object' &&
    value !== null &&
    Array.isArray(value['Fn::Equals']) &&
    value['Fn::Equals'].length === 2
  );
}
...
function deepEquals(
  left: PropertyValue | undefined,
  right: PropertyValue | undefined,
): boolean {
  if (left === right) {
    return true;
  }
  ...
  if (Array.isArray(left) && Array.isArray(right)) {
    ...
    return left.every((item, idx) =>
      deepEquals(item as PropertyValue, right[idx] as PropertyValue),
    );
  }
  ...
  if (isObject(left) && isObject(right)) {
    ...
    return leftKeys.every((key) =>
      deepEquals((left as any)[key], (right as any)[key]),
    );
  }
