...
export function convertStacksToProgramIr(
  stacks: StackConversionInput[],
): ProgramIR {
  return {
...
export function convertStackToIr(
  input: StackConversionInput,
  options?: StackIrConversionOptions,
): StackIR {
...
function buildResourceOptions(
  ...
  if (dependsOn && dependsOn.length > 0) {
    options.dependsOn = dependsOn.map(
      (id): StackAddress => ({
        id,
        stackPath,
      }),
