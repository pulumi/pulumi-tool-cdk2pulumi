...
const SSM_PLAINTEXT_DYNAMIC_REGEX =
  /^{{resolve:ssm:([a-zA-Z0-9_.\-/]+(?::\d+)?)}}$/;
const SSM_SECURE_DYNAMIC_REGEX =
  /^{{resolve:ssm-secure:([a-zA-Z0-9_.\-/]+(?::\d+)?)}}$/;

const SECRETS_MANAGER_DYNAMIC_REGEX =
  /\{\{resolve:secretsmanager:([^:]+(?::[^:]+)*?)(?::([^:]*))?(?::([^:]*))?(?::([^:]*))?(?::([^:]*))?\}\}/;

export function tryParseDynamicReference(
  value: string,
): DynamicReferenceValue | undefined {
...
function parseSsmPlaintextReference(
  value: string,
): SsmDynamicReferenceValue | undefined {
...
function parseSsmSecureReference(
  value: string,
): SsmDynamicReferenceValue | undefined {
...
function parseSecretsManagerReference(
  value: string,
): SecretsManagerDynamicReferenceValue | undefined {
