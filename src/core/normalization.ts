...
export function normalizeResourceProperties(
  value: any,
  options?: NormalizeResourceOptions,
): any {
...
function normalizeObject(
  ...
  try {
    ...
    for (const [rawKey, rawVal] of Object.entries(value)) {
      const nextKey =
        nativeType === NativeType.ADDITIONAL_PROPERTIES
          ? rawKey
          : toSdkName(rawKey);
...
function normalizeGenericResourceObject(
  key: string[],
  value: Record<string, any>,
  options?: NormalizeResourceOptions,
) {
  const result: any = {};
  for (const [rawKey, rawVal] of Object.entries(value)) {
    result[toSdkName(rawKey)] = normalizeValue(
      [...key, rawKey],
      rawVal,
      options,
    );
