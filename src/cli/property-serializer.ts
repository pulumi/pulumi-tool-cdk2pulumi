  ...
  if (isPropertyMap(value)) {
    return Object.fromEntries(
      Object.entries(value).map(([key, nested]) => [
        key,
        serializePropertyValue(nested, ctx),
      ]),
    );
  }
...
function serializeResourceAttributeReference(
  ...
  return propertyPart
    ? `\${${resourceName}.${propertyPart}}`
    : `\${${resourceName}}`;
}

function serializeStackOutputReference(
  value: StackOutputReference,
  ctx: PropertySerializationContext,
): string {
  const outputName = ctx.getStackOutputName(value.stackPath, value.outputName);
  if (!outputName) {
    throw new Error(
      `Failed to resolve stack output ${value.outputName} in stack ${value.stackPath}`,
    );
  }
  return `\${${outputName}}`;
}

function serializeParameterReference(
...
function serializeSecretsManagerDynamicReference(
  value: SecretsManagerDynamicReferenceValue,
) {
  if (value.jsonKey) {
    throw new Error(
  ...
  const invokeExpression = {
    'fn::invoke': {
      ...
      return:
        value.secretString === 'SecretBinary' ? 'secretBinary' : 'secretString',
    },
  };
